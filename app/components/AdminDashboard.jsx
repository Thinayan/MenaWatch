...
import ArchiveImporter from "./admin/ArchiveImporter";
...
function ArchiveSection() {
  const [activeView, setActiveView] = useState("importer"); // importer | catalog
  const [stats, setStats] = useState({ events: "—", indicators: "—" });

  useEffect(() => {
    // Fetch archive stats
    fetch("/api/archive/events?limit=0").then(r => r.json()).then(d => {
      setStats(prev => ({ ...prev, events: d.total || 0 }));
    }).catch(() => {});
    fetch("/api/archive/indicators?country=SA&indicator=gdp").then(r => r.json()).then(d => {
      setStats(prev => ({ ...prev, indicators: d.data?.length || 0 }));
    }).catch(() => {});
  }, []);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
        {[
          { l:"أحداث تاريخية", v: stats.events, c:"#3b82f6" },
          { l:"نقاط مؤشرات", v: stats.indicators, c:"#22c55e" },
          { l:"صفحة الأرشيف", v:"✓", c:"#8b5cf6" },
        ].map(s => (
      ...
      <div style={{ display:"flex", gap:8, marginBottom:4 }}>
        <button onClick={() => setActiveView("importer")} style={{ padding:"6px 14px", borderRadius:6, fontSize:11, fontFamily:"inherit", cursor:"pointer", border:`1px solid ${activeView==="importer"?"#22c55e":"#1e293b"}`, background:activeView==="importer"?"#22c55e18":"transparent", color:activeView==="importer"?"#22c55e":"#94a3b8", fontWeight:activeView==="importer"?700:500 }}>
          📥 استيراد بيانات
        </button>
        <button onClick={() => setActiveView("catalog")} style={{ padding:"6px 14px", borderRadius:6, fontSize:11, fontFamily:"inherit", cursor:"pointer", border:`1px solid ${activeView==="catalog"?"#22c55e":"#1e293b"}`, background:activeView==="catalog"?"#22c55e18":"transparent", color:activeView==="catalog"?"#22c55e":"#94a3b8", fontWeight:activeView==="catalog"?700:500 }}>
          📚 الأرشيف المتاح
        </button>
        <a href="/archive" target="_blank" style={{ marginRight:"auto", padding:"6px 14px", borderRadius:6, fontSize:11, textDecoration:"none", border:"1px solid #3b82f644", color:"#3b82f6", display:"flex", alignItems:"center", gap:4 }}>
          🔗 عرض الأرشيف
        </a>
      </div>

      {activeView === "importer" && <ArchiveImporter />}
...
export default function AdminDashboard() {
  ...
  return (
    <div className="admin-root" style={{ fontFamily:"'IBM Plex Sans Arabic','Tajawal',sans-serif", background:"#060d18", minHeight:"100vh", color:"#e2e8f0", direction:"rtl", display:"flex", flexDirection:"column" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@300;400;500;600;700&display=swap');
        .admin-root, .admin-root * { box-sizing:border-box; margin:0; padding:0; }
